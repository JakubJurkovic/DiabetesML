---
title: "Biological Dataased & Datamining : Final"
description: |
...
The following document is a final project fro Biological Databases & Datamining, in which we will attempt to build and evaluate mulptiple models to predict diabetes in patients. The caveat is that we expect our training data to cotain a lot of missing data.
...
# Q1 Which variables are numeric and which are categorical?
...
* For each variable draw an appropriate graph ( boxplot for numerical values and barplot for categorical values).

```{r VariablesInspection, fig.width=8}
...
data_long %>% ggplot(mapping = aes(x=as.factor(Outcome))) +
  geom_bar() +
  theme_minimal() +
  labs(
    x = "Outcome",
...
The jittered point plot reveals some clear clusters being observable between the diagnosis. The most notable one is in the `Glucose` & `BMI` plot, which is also confirmed by the correlation heatmap - these should be the most contributing variables to the predictions.
...
```{r Imputing, fig.width=8}
# Q3 Ctd

# Run the KNN Imputer and bring back the outcome column
data_na <- impute.knn(as.matrix(data_na), k = 15, rowmax = 0.5, colmax = 0.8, maxp = 1500, rng.seed=362436069)
...
Looking at our violin plots their general "shapes" have remained almost the same, with the "tails" caused by excess NAs in the form have been de-facto trimed. This means that our imputer mantained the relative distribution of our values. We can also observe change in the correlation heatmap, boosting the importance of some factors that were otherwise masked by the high number of 0 present.
...
The best performer will be then chosen based on the the mixutre of highest and least variable AUC.  

## K-Fold Validation & AUC

In order to utlize 100% of our dataset let's perform the k-fold validation. We can use `vfold_cv` command from base R.
...
The hypertuning shows that the best parameters given this dataset for the cforest seesm to be 500 trees and 2 randomly selected predictors. With the best set of predictors 
...
The KNN's hypertuning through grid search revealed that the performance plateu's at k=50, however most of the imprtance is achieved at k=29

## Neural Networks

```{r NeuralNetworks, fig.width=8}
...
Hyperturning of the neural network shows that the model with 1 hidden unit had the most consistent performance. Although some other models peaked above that one, it has been less consistent overall.
